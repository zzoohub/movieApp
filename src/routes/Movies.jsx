...
const Slide = styled.div`
  ...
  transform: translateX(${props => (props.translate === 'base' ? 10 : -props.translate)}vw);
  height: 100%;
...
export default function Movies() {
  ...
  const { data: upcoming, isLoading: upcomingLoading } = useQuery(['movie', 'upcoming'], getUpcomingMovies);
  const { data: topRated, isLoading: topRatedLoading } = useQuery(['movie', 'toprated'], getTopRatedMovies);
      ...
      {upcomingLoading || topRatedLoading ? (
        ...
        <Wrapper>
          <Banner>
                ...
                {popula?.map((movie, i) => (
                  <Box key={i} active={index === i ? true : false}>
                    <Link to={`/movies/${movie.id}`}>
                      <img src={movie.backdrop_path ? makeImgPath(movie.backdrop_path) : ''}></img>
                    </Link>
            ...
            <PageNation>
              {[...Array(20)].map((box, i) => (
                <div key={i} onClick={() => setIndex(i + 1)} className={i + 1 === index ? 'on' : ''}></div>
              ))}
            </PageNation>
          </Banner>
          <TopRated>
            ...
            <SlideMulti offset={5} data={topRated?.results} type="movie"></SlideMulti>
          </TopRated>
          <Trending>
            <Title>주간 트렌드</Title>
            <InfiniteSlide
