...
export default function TvDetail() {
  const { id } = useParams();
  const { data, isLoading, refetch } = useQuery(['tv', 'detail'], () => getTvDetail(id));
  const {
    data: similarData,
  ...
  useEffect(() => {
    ...
    fetch(`https://api.themoviedb.org/3/tv/${id}/videos?api_key=${process.env.REACT_APP_API_KEY}&language=en-US`)
      .then(res => res.json())
      .then(json => {
        if (json.results[0]) {
